...
from Topo_Specs_General_2 import switch_port_mac_dict as mac_dict,dpid_switch_number as sw_num,switch_port_mat, switch_number_dpid as sw_dpid, switch_dict, paths as path_list
...
class RouteManagement():
    ...
    def createFlow(self, path):
        ...
        for i in range(dst):
            ...
            forward_flow1 = '{"switch":"'+sw_dpid[path[i]]+'","name":"'+str(self.flow_counter)+'", "eth_dst":"fa:16:3e:00:14:17", "eth_type":"0x0800", "ip_dscp":"40", "enqueue":"'+forward_flow_egress_port+':0", "actions":"output='+forward_flow_egress_port+'"}'
            self.flow_counter += 1
            forward_flow2 = '{"switch":"' + sw_dpid[path[i]] + '","name":"' + str(self.flow_counter) + '", "enqueue":"'+forward_flow_egress_port+':0", "eth_dst":"fa:16:3e:00:14:17", "actions":"output=' + forward_flow_egress_port + '"}'
            self.flow_counter += 1
            ...
            backward_flow1 = '{"switch":"'+sw_dpid[path[i+1]]+'","name":"'+str(self.flow_counter)+'", "eth_dst":"fa:16:3e:00:1b:7c", "eth_type":"0x0800", "enqueue":"'+backward_flow_egress_port+':0", "ip_dscp":"40", "actions":"output='+backward_flow_egress_port+'"}'
            self.flow_counter += 1
            backward_flow2 = '{"switch":"'+sw_dpid[path[i+1]]+'","name":"'+str(self.flow_counter)+'", "enqueue":"'+backward_flow_egress_port+':1","eth_dst":"fa:16:3e:00:1b:7c", "actions":"output='+backward_flow_egress_port+'"}'
            self.flow_counter += 1
            backward_flow3 = '{"switch":"'+sw_dpid[path[i+1]]+'","name":"'+str(self.flow_counter)+'", "enqueue":"'+backward_flow_egress_port+':0", "eth_dst":"fa:16:3e:00:45:76", "actions":"output='+backward_flow_egress_port+'"}'
            self.flow_counter += 1
    ...
    def createEndpointFlow(self,src,dst):
        ...
        flows = defaultdict(list)
        flow1 = '{"switch":"00:00:72:40:b2:ac:47:4b","name":"0", "eth_dst":"72:40:b2:ac:47:4b", "eth_type":"0x0800", "enqueue":"2:0", "ip_dscp":"40", "actions":"output=4"}'
        flow2 = '{"switch":"00:00:72:40:b2:ac:47:4b","name":"1", "eth_dst":"72:40:b2:ac:47:4b", "enqueue":"2:1", "actions":"output=4"}'
        flows[sw_dpid[src]].append(flow1)
        flows[sw_dpid[src]].append(flow2)
        #Leaving Multimedia server2 out of QoS categorization
        flow = '{"switch":"00:00:72:40:b2:ac:47:4b","name":"2", "eth_dst":"fa:16:3e:00:45:76", "enqueue":"2:1", "actions":"output=2"}'
        flows[sw_dpid[src]].append(flow)
        #Flow for client
        flow1 = '{"switch": "00:00:da:e7:78:d1:30:44", "name": "3", "eth_dst": "fa:16:3e:00:14:17", "eth_type":"0x0800", "ip_dscp":"40", "enqueue":"1:0", "actions": "output=2"}'
        flow2 = '{"switch": "00:00:da:e7:78:d1:30:44", "name": "4", "eth_dst": "fa:16:3e:00:14:17", "enqueue":"1:1", "actions": "output=2"}'
